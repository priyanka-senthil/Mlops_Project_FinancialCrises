...
class StressTestPipeline:
    ...
    def _generate_single_scenario(self, sigma: float) -> Dict:
        """Generate single scenario using VAE decoder - FIXED VERSION"""
        
        vae_model = self.model1.get("vae")
        scaler = self.model1.get("scaler")
        feature_names = self.model1.get("features", [])
        
        # Check if we have the necessary components
        if not vae_model or not feature_names:
            logger.error("❌ VAE model or feature names not found in model1")
            raise ValueError("VAE model not properly loaded")
        
        # Get latent dimension from VAE
        latent_dim = vae_model.latent_dim if hasattr(vae_model, 'latent_dim') else 32
        
        logger.info(f"🔍 Generating scenario with sigma={sigma}, latent_dim={latent_dim}")
        
        try:
            # Sample from latent space
            z = torch.randn(1, latent_dim) * sigma
            
            # Decode using VAE
            with torch.no_grad():
                z_decoded = vae_model.decode(z).numpy()[0]
            
            logger.info(f"🔍 VAE decoded shape: {z_decoded.shape}, size: {z_decoded.size}")
            
            # Check if scaler exists and is compatible
            if scaler is not None:
                # Verify scaler has correct number of features
                expected_features = scaler.n_features_in_ if hasattr(scaler, 'n_features_in_') else len(feature_names)
                
                logger.info(f"🔍 Scaler expects {expected_features} features, decoded has {z_decoded.size}")
                
                if z_decoded.size == 0:
                    # VAE output is empty - use fallback
                    logger.warning("⚠️ VAE decoder output is empty, using fallback generation")
                    generated_values = self._generate_fallback_scenario_values(sigma, len(feature_names))
                    
                elif z_decoded.size == expected_features:
                    # Perfect match - inverse transform
                    generated_values = scaler.inverse_transform(z_decoded.reshape(1, -1))[0]
                    
                else:
                    # Size mismatch - pad or truncate
                    logger.warning(f"⚠️ Size mismatch: adjusting from {z_decoded.size} to {expected_features}")
                    if z_decoded.size < expected_features:
                        # Pad with zeros
                        z_padded = np.zeros(expected_features)
                        z_padded[:z_decoded.size] = z_decoded
                        z_decoded = z_padded
                    else:
                        # Truncate
                        z_decoded = z_decoded[:expected_features]
                    
                    generated_values = scaler.inverse_transform(z_decoded.reshape(1, -1))[0]
            else:
                # No scaler - use decoded values directly
                logger.info("ℹ️ No scaler found, using VAE output directly")
                if z_decoded.size == 0:
                    generated_values = self._generate_fallback_scenario_values(sigma, len(feature_names))
                else:
                    generated_values = z_decoded
            
            # Ensure we have the right number of features
            if len(generated_values) < len(feature_names):
                logger.warning(f"⚠️ Not enough values ({len(generated_values)}) for features ({len(feature_names)})")
                generated_values = self._generate_fallback_scenario_values(sigma, len(feature_names))
            
            # Create dict with feature names
            scenario_dict = {}
            for i, name in enumerate(feature_names[:len(generated_values)]):
                scenario_dict[name] = float(generated_values[i])
            
            logger.info(f"✅ Generated scenario with {len(scenario_dict)} features")
            
            return scenario_dict
            
        except Exception as e:
            logger.error(f"❌ VAE generation failed: {e}")
            logger.info("📝 Using complete fallback scenario generation")
            return self._generate_complete_fallback_scenario(sigma)
    
    def _generate_fallback_scenario_values(self, sigma: float, n_features: int) -> np.ndarray:
        """Generate fallback values when VAE fails"""
        logger.info(f"📝 Generating {n_features} fallback values")
        
        # Common economic feature order (adjust based on your actual features)
        baseline = np.array([
            16000,  # GDP
            250,    # CPI
            5.0,    # Unemployment
            ...
            0.05,   # SP500 Return
            70,     # Oil Price
            0.02,   # Corporate Bond Spread
            0,      # Financial Stress Index
            0.03    # GDP Growth
        ])
        
        # Extend or truncate to match needed features
        if n_features > len(baseline):
            # Add more random features
            extra = np.random.randn(n_features - len(baseline)) * sigma * 10
            baseline = np.concatenate([baseline, extra])
        else:
            baseline = baseline[:n_features]
        
        # Add stress based on sigma
        stress = np.random.randn(n_features) * sigma * baseline * 0.1
        
        return baseline + stress
    
    def _generate_complete_fallback_scenario(self, sigma: float) -> Dict:
        """Generate complete fallback scenario with proper feature names"""
        logger.info("📝 Using complete fallback scenario generation")
        
        # Stress multiplier based on sigma
        stress = 1 + sigma * 0.2
        
        # Base economic indicators
        scenario = {
            "GDP": 16000 / stress,
            "GDP_Growth_252D": 0.03 / stress,
            "CPI": 250 * (1 + sigma * 0.01),
            "Inflation": 0.02 * stress,
            "Unemployment_Rate": 5.0 * stress,
            "VIX": 20 * stress,
            "SP500_Close": 4500 / stress,
            "SP500_Return_90D": 0.05 / stress,
            "Oil_Price": 70 * (1 + np.random.randn() * 0.1),
            "Federal_Funds_Rate": 2.5 * (1 + sigma * 0.1),
            "Corporate_Bond_Spread": 0.02 * stress,
            "Financial_Stress_Index": sigma * 0.5,
        }
    ...
    def _run_single_scenario(
        ...
        # STEP 3: Run Model 2 predictions
        logger.info(f"   → Running Model 2 predictions...")
        ...
        # STEP 4: Prepare Model 3 input (14 features)
        model3_input = self._prepare_model3_input(
            vae_features=vae_features,
            predictions=predictions,
            company_current=latest
    ...
    def _calculate_shap_explanations(self, X_model3: pd.DataFrame) -> List[Dict]:
        ...
        try:
            ...
            # Normalize SHAP values to -100 to +100
            max_abs_shap = max(abs(e["shap_value"]) for e in explanations)
            ...
            if max_abs_shap > 0:
                for exp in explanations:
                    raw_value = exp["shap_value"]
                    normalized_value = (raw_value / max_abs_shap) * 100
                    exp["shap_value"] = normalized_value
            
            logger.info(f"   ✓ SHAP calculated")
            return explanations
            
        except Exception as e:
            logger.warning(f"   ⚠️ SHAP failed: {e}, using fallback")
            return self._fallback_shap_explanations(X_model3)
    
    def _fallback_shap_explanations(self, X_model3: pd.DataFrame) -> List[Dict]:
        """Fallback SHAP explanations when calculation fails"""
        explanations = []
        for i, feature in enumerate(self.config.MODEL3_FEATURES):
            feature_val = float(X_model3.iloc[0, i])
            ...
            # Estimate impact
            if 'vix' in feature.lower():
                impact = (feature_val - 20) * 2
            elif 'unemployment' in feature.lower():
                impact = (feature_val - 5) * 3
            elif 'gdp' in feature.lower():
                impact = (16000 - feature_val) * 0.002
            elif 'debt' in feature.lower():
                impact = (feature_val - 1.5) * 5
            elif 'margin' in feature.lower() or 'roa' in feature.lower() or 'roe' in feature.lower():
                impact = (0.1 - feature_val) * 50
            else:
                impact = 0
    ...
    def _get_contribution_description(self, shap_value: float) -> str:
        """Get human-readable description of SHAP contribution"""
        abs_val = abs(shap_value)
        direction = "increases" if shap_value > 0 else "decreases"
    ...
    def _calculate_risk_score(self, anomaly_score: float, anomaly_pred: int) -> float:
        """Convert anomaly score to 0-100 risk score"""
        if anomaly_score < -5.0:
            risk = 95 + min(abs(anomaly_score + 5.0), 5)
        elif anomaly_score < -3.0:
            risk = 75 + (abs(anomaly_score + 3.0) / 2.0) * 20
        elif anomaly_score < -1.0:
            risk = 50 + (abs(anomaly_score + 1.0) / 2.0) * 25
        elif anomaly_score < 0.0:
            risk = 30 + abs(anomaly_score) * 20
        elif anomaly_score < 0.3:
            risk = 15 + (anomaly_score / 0.3) * 15
        else:
            risk = max(0, 15 - (anomaly_score - 0.3) * 20)
